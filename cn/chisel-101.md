...
注意到，由于 Chisel 语言只有 0/1，不支持三态或者高阻态，所以这里对几个寄存器采取了随机初始化的办法。另外也可以看到，尽管模块中没有声明时钟 `clock` 和重置 `reset` 信号，但是所有编译出来的 Verilog 代码都会包含这两个输入，å¹¶且在有需要的时候向嵌套的模块传递，这可以在下面这个示例中观察到。
